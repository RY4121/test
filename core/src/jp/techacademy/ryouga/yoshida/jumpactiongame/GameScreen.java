...
import static jp.techacademy.ryouga.yoshida.jumpactiongame.Enemy.ENEMY_TYPE_MOVING;

public class GameScreen extends ScreenAdapter{
    //Sound sound = Gdx.audio.newSound(Gdx.files.internal("assets/sound.wav"));
    Music music = Gdx.audio.newMusic(Gdx.files.internal("sound.wav"));
    ...
    //20画面分登れば終了
    static final float WORLD_HEIGHT = 15 * 5;
    ...
    //ScreenAdapterクラスの上で表示されるオブジェクトがSprite
    Sprite mBg;
    ...
    public GameScreen(JumpActionGame game) {
        ...
        mPrefs = Gdx.app.getPreferences("jp.techacademy.ryouga.yoshida.jumpactiongame");
        mHighScore = mPrefs.getInteger("HIGHSCORE", 0);

        createStage();//オブジェクトを配置するメソッド
    }
    ...
    public void render(float delta) {
        ...
        if (mPlayer.getY() > mCamera.position.y) { //カメラの中心座標
        ...
        //Enemy
        for (int i = 1; i < mEnemies.size(); i+=2) {
    ...
    private void createStage() {
        ...
        while (y < WORLD_HEIGHT - 5) {
            ...
            int typeEnemy = ENEMY_TYPE_MOVING;
            ...
            Enemy enemy = new Enemy(typeEnemy, enemyTexture, 0, 0, 120, 50);
    ...
    private void updatePlaying(float delta) {
        ...
        if (Gdx.input.isTouched()) {
            ...
            Rectangle left = new Rectangle(0, 0, CAMERA_WIDTH / 2, GUI_HEIGHT);
            Rectangle right = new Rectangle(CAMERA_WIDTH / 2, 0, CAMERA_WIDTH / 2, GUI_HEIGHT);
    ...
    private void checkCollision() {
        ...
        for (int i = 1; i < mEnemies.size(); i+=2) {
            ...
            boolean EnemyCollision = mPlayer.getBoundingRectangle().overlaps(enemy.getBoundingRectangle());
            if (EnemyCollision) {
                // mPlayer.hitStep();
                flag = true;
            }
    ...
    private void checkGameOver() {
        if (mHeightSoFar - CAMERA_HEIGHT / 2 > mPlayer.getY() || flag) {
